...
int main(int argc, char *argv[])
{
    ...
    switch (flag)
    {
        ...
        case LEXICOGRAPHY | FOLD | REVERSE:
            ...
            break;
        case LEXICOGRAPHY | DIRECTORY:
            myqsort(lines, 0, line_size, d_strcmp);
            break;
        case LEXICOGRAPHY | DIRECTORY | FOLD:
            myqsort(lines, 0, line_size, f_d_strcmp);
            break;
    }
    putchar('\n');
    for(int i = 0; i < line_size; i++)
...
int d_strcmp(char *a, char *b)
{
    void str2director(char *s);
    char *a_d = malloc(sizeof(char) * (strlen(a) + 1));
    strcpy(a_d, a);
    str2director(a_d);
    char *b_d = malloc(sizeof(char) * (strlen(b) + 1));
    strcpy(b_d, b);
    str2director(b_d);
    int res = strcmp(a_d, b_d);
    free(a_d);
    free(b_d);
    return res;
}
...
void str2director(char *s)
{
    int left, right;
    int size = strlen(s);
    for(left = right = 0; right < size; right++)
        if(isalnum(s[right]) == 0 || isspace(s[right]) == 0)
            s[left++] = s[right];
}
